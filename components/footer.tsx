...
export default function Footer() {
  return (
    <footer className="w-full border-ali/20 border-t border-b bg-white px-2 dark:bg-black">
      <div className="relative mx-auto grid max-w-7xl items-center justify-center gap-6 p-10 pb-0 md:flex">
        ...
        <p className="bg-transparent text-center text-primary/60 text-xs leading-4 md:text-left dark:text-gray-400">
            ...
            >
              {category.sections.map((section) => (
                <div key={section.name}>
                  <h3 className="mb-3 font-semibold text-red-600 dark:text-red-500">
                    {section.name}
                  </h3>
                  <ul
                  ...
                  >
                    {section.items.map((item) => (
                      <li className="flow-root" key={item.name}>
                        <Link
                          className="text-slate-600 text-sm hover:text-black md:text-xs dark:text-slate-400 hover:dark:text-white"
                          href={item.href}
                        >
                          {item.name}
                        </Link>
